 ...
 * @returns {string}
 */
export const formatRemoveSpace = (value: string = "") =>
    value.replace(/\s+/g, "");
 ...
 * @returns {string}
 */
export const formatRemoveExtraSpaces = (value: string = "") =>
 ...
 * Slugify a string
 */
export const formatSlugify = (value: string = "") =>
    value.replace(/\s+/g, "_").toLowerCase();

/**
 * Convert a 'a_text' string to 'a text'
 */
export const formatUnUnderscore = (value: string = "") =>
    value.replace(/_/g, " ");
...
/**
 * Format string to coded format (something-word-wow)
 *
 * @param {string} value - The value to codify
 * @returns {string} - The codified value
 */
export const formatCodedString = (value?: string | null) => {
    let code = value ?? "";

    for (const rule of JSON_Rules.replace.code.rules) {
        const regex = new RegExp(rule?.regex, rule.flags);
        code = code.replace(regex, rule.replacement);
    }
